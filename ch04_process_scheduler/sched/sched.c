...
static void child_fn(
	int id, struct timespec *buf, int nrecord, unsigned long nloop_per_resol, struct timespec start
){
	int i;
	for(i = 0; i < nrecord; i++)
	{
...
static void parent_n(int nproc)
{
	int i;
	for(i = 0; i < nproc; i++)
	{
...
int main(int argc, char *argv[])
{
	...
	if (total % resol){
		fprintf(
			stderr,
			"<total> (%d) should be multiple of <resol> (%d).\n",
			total, resol
		);
		exit(EXIT_FAILURE);
	}
